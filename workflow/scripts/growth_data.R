...
#  geom_signif(comparisons = list(c("Ancestor", "BAC_21")), map_signif_level = TRUE) +
#  geom_signif(comparisons = list(c("Ancestor", "BAC_22")), map_signif_level = TRUE) +
...
ggplot(df5, aes(x = type, y = finalOD)) + geom_boxplot() + 
  ylab(expression(paste("final ", "OD"["600"], sep = " "))) + 
  xlab("") +
  facet_wrap(~ type2, scales = "free_x", ncol = 5) + 
  geom_signif(comparisons = shcomp[1:3], map_signif_level = TRUE, step_increase = 0.01)
